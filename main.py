...
async def reg(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_text(text="Привет, " + user.username + "! Введите ваше имя и количество продаж")
...
async def static(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_text(text=str(curr))


def main() -> None:
    app = Application.builder().token(Token).build()

    app.add_handler(CommandHandler("start", reg))
